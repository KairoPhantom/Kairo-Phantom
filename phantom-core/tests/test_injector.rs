...
fn test_concurrent_clipboard_mutex_safety() {
    ...
    assert!(!finished.load(Ordering::SeqCst), "Background thread should be blocked by CLIPBOARD_MUTEX");
    ...
    assert!(finished.load(Ordering::SeqCst), "Background thread should have finished after lock release");
}
