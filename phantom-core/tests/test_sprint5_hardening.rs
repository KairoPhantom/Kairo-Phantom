...
mod test_sprint5_hardening {
    use phantom_core::pro::{KairoPro, TolariaBridge, AuditExport, unlock_advanced_agents, TeamMemoryVault, TEAM_MEMORY_VAULT_ERR, AUDIT_EXPORT_ERR};
    ...
    fn test_inject_guidelines_returns_err_when_not_pro() {
        ...
        assert!(msg.contains("unavailable") || msg.contains("Kairo Pro"),
    ...
    fn test_unlock_advanced_agents_returns_err_when_not_pro() {
        ...
        assert!(result.is_err(), "unlock_advanced_agents must Err when not Pro");
        let msg = result.unwrap_err();
        assert!(msg.contains("unavailable") || msg.contains("Kairo Pro"),
            "Error must mention unavailability: got '{}'", msg);
    }
    ...
    fn test_audit_export_error_message_is_descriptive() {
        let result = AuditExport::export_csv(
            &KairoPro::new(), "user", "app", "agent", "hash", "outcome", 100
        );
    ...
    fn make_real_agent(id: &str) -> Arc<dyn phantom_core::plugin::SwarmAgent> {
        ...
        struct RealAgent { id_str: String }
        impl phantom_core::plugin::SwarmAgent for RealAgent {
            fn id(&self) -> &str { &self.id_str }
            fn name(&self) -> &str { "Real Agent" }
            fn build_system_prompt(&self, _: &DocumentContext) -> String { String::new() }
            fn match_score(&self, _: &DocumentContext) -> u8 { 50 }
            fn capability(&self) -> DomainCapability { DomainCapability::Real }
        }
        Arc::new(RealAgent { id_str: id.to_string() })
    }

    fn make_prompt_only_agent(id: &str) -> Arc<dyn phantom_core::plugin::SwarmAgent> {
        struct ThinAgent { id_str: String }
        impl phantom_core::plugin::SwarmAgent for ThinAgent {
            fn id(&self) -> &str { &self.id_str }
            fn name(&self) -> &str { "Thin Agent" }
            fn build_system_prompt(&self, _: &DocumentContext) -> String { String::new() }
            fn match_score(&self, _: &DocumentContext) -> u8 { 10 }
            fn capability(&self) -> DomainCapability { DomainCapability::PromptOnly }
        }
        Arc::new(ThinAgent { id_str: id.to_string() })
    }
    ...
    fn test_public_agents_strips_prompt_only() {
        ...
        assert_eq!(public.len(), 2, "Only 2 Real agents should appear in public list");
        for agent in &public {
            assert_eq!(agent.capability(), DomainCapability::Real,
    ...
    fn test_capability_map_includes_all_with_correct_capabilities() {
        ...
        let word_cap = cap_map.iter().find(|(id, _)| id == "word-agent").map(|(_, c)| c);
        let med_cap = cap_map.iter().find(|(id, _)| id == "medical-thin").map(|(_, c)| c);
    ...
    async fn test_api_get_capabilities() {
        use phantom_core::api::{ApiState, get_capabilities};
        ...
        for cap in list {
            assert_eq!(cap.capability, phantom_core::plugin::DomainCapability::Real, "Should only return Real capability agents");
        }
    }
}
