...
impl DeepPresenter {
    ...
    pub async fn generate(
        ...
        let slide_count = if outline.is_empty() { 8 } else { outline.len() + 2 };
        ...
        tracing::info!(
            ...
            dp_resp.slides.len(), dp_resp.generation_time_ms
        );
        Ok(dp_resp.slides)
    }
    ...
    pub fn generate_fallback(topic: &str, outline: &[String]) -> Vec<SlideContent> {
        ...
        for (i, section) in outline.iter().enumerate() {
            slides.push(SlideContent {
                ...
                speaker_notes: Some(format!("Section {} of {}: {}", i + 1, outline.len(), section)),
