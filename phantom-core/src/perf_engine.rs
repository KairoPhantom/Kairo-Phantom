...
impl SIMDTextDiffer {
    ...
    pub fn find_divergence(&self, current: &str, proposed: &str) -> Option<usize> {
        ...
        while i + 16 <= min_len {
            let mut xor_sum = 0u8;
            for j in 0..16 { xor_sum |= cb[i+j] ^ pb[i+j]; }
            if xor_sum != 0 {
                for j in 0..16 {
                    if cb[i+j] != pb[i+j] { return Some(i + j); }
                }
            }
            i += 16;
        }
        while i < min_len {
            if cb[i] != pb[i] { return Some(i); }
            i += 1;
        }
        if cb.len() != pb.len() { Some(min_len) } else { None }
    }
    ...
    pub fn count_tokens(&self, text: &str) -> usize {
        ...
        for &b in text.as_bytes() {
            if b == b' ' || b == b'\n' || b == b'\t' { prev_sep = true; }
            else if prev_sep { count += 1; prev_sep = false; }
        }
        count
    }
...
impl Default for SIMDTextDiffer { fn default() -> Self { Self::new() } }
            ...
            match memchr(b'\n', &self.buf[start..]) {
                ...
                Some(nl) => {
                    ...
                    if line.starts_with(b"data:") {
                        let raw = &line[5..];
                        let value = if raw.starts_with(b" ") { &raw[1..] } else { raw };
                        if value != b"[DONE]" {
        ...
        for pat in patterns {
            if let Some(start) = find_pattern(data, pat) {
                ...
                while end < content.len() {
                    if content[end] == b'"' && (end == 0 || content[end-1] != b'\\') { break; }
                    end += 1;
                }
...
struct CachedModel { last_used: u64, reuse_count: u64 }
...
impl McpModelCache {
    pub fn new() -> Self {
        Self { instances: std::sync::Mutex::new(std::collections::HashMap::new()) }
    }
    ...
    pub fn get_or_create(&self, model_name: &str) -> bool {
        ...
        if let Some(e) = cache.get_mut(model_name) {
            if now - e.last_used < 300 {
                e.last_used = now;
                e.reuse_count += 1;
                tracing::debug!("[McpCache] Hit: {} (reused {} times)", model_name, e.reuse_count);
                return true; // cache hit
            }
        }
        cache.insert(model_name.to_string(), CachedModel { last_used: now, reuse_count: 0 });
    ...
    pub fn evict_expired(&self) {
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default().as_secs();
        self.instances.lock().unwrap().retain(|_, v| now - v.last_used < 300);
    }

    pub fn size(&self) -> usize { self.instances.lock().unwrap().len() }
}
...
#[derive(Debug, Clone)]
pub enum BatchOp {
    ReadContext { text: String },
    DetectApp { title: String },
    GhostWrite { prompt: String, context: String, app: String },
}
...
pub async fn execute_batch(ops: Vec<BatchOp>) -> Vec<BatchResult> {
    ...
    for op in ops {
        ...
        match op {
            ...
            BatchOp::GhostWrite { prompt, context, app } => {
                results.push(BatchResult {
                    op_type: "ghost_write".into(),
                    success: true,
                    data: Some(format!("[pipeline] app={} ctx={}chars prompt={}", app, context.len(), &prompt[..prompt.len().min(40)])),
                    latency_ms: start.elapsed().as_millis(),
                });
            }
        }
...
pub async fn parallel_agent_calls(
    ...
    let futs: Vec<_> = calls.into_iter().map(|(id, sys, usr)| {
        let b = backend.clone();
        async move {
...
mod tests {
    ...
    fn test_token_extract_openai() {
        ...
        assert_eq!(ZeroAllocSseParser::extract_token_fast(data), Some("Hi".into()));
    }
