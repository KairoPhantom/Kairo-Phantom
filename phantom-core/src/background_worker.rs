...
pub async fn start_document_scanner(mem_store: Arc<std::sync::Mutex<(MemoryStore, crate::memory::KairoMemory)>>) {
        ...
        match fs::read_dir(&inbox_path) {
            Ok(entries) => {
                for entry in entries.flatten() {
                    let path = entry.path();
                    if path.is_file() {
                        info!("📄 Background Worker: Processing new document: {:?}", path.file_name());
                        ...
                        if let Ok(mut store_lock) = mem_store.lock() {
                            ...
                            memory.app_bias.insert(filename.clone(), format!("Context from background document: {}", filename));
                        }
