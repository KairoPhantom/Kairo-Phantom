...
impl LanSync {
    pub fn new(vault_dir: &Path) -> Self {
        Self { db_path: vault_dir.join("mem_machine.db") }
    }
    ...
    pub fn discover_peers(&self, timeout_ms: u64) -> Vec<LanPeer> {
        ...
        let _ = sock.send_to(announce.as_bytes(), format!("255.255.255.255:{}", DISCOVERY_PORT));
    ...
    pub fn start_server(&self) -> Result<()> {
        ...
        for stream in listener.incoming().flatten() {
            ...
            std::thread::spawn(move || { let _ = handle_sync_client(stream, &db); });
        }
        Ok(())
    }
    ...
    pub fn pull_from_peer(&self, peer_addr: &str) -> Result<usize> {
        ...
        if data.len() < 12 { return Ok(0); }
        let packet: SyncPacket = bincode::deserialize(&data)
            .map_err(|e| anyhow::anyhow!("Deserialization failed: {}", e))?;

        if packet.magic != MAGIC { return Ok(0); }

        let merged = self.merge_entries(&packet.entries)?;
        tracing::info!("🌐 LAN sync: merged {} new entries from {}", merged, peer_addr);
        Ok(merged)
    }

    fn count_entries(&self) -> Result<u64> {
        if !self.db_path.exists() { return Ok(0); }
        let conn = rusqlite::Connection::open(&self.db_path)?;
        let n = conn.query_row("SELECT COUNT(*) FROM semantic_memory", [], |r| r.get::<_, i64>(0))
            .unwrap_or(0);
        Ok(n as u64)
    }
    ...
    fn export_entries(&self) -> Result<Vec<SyncEntry>> {
        ...
        let rows = stmt.query_map([], |row| Ok(SyncEntry {
            id: row.get(0)?, content: row.get(1)?,
            app_context: row.get(2)?, context_key: row.get(3)?,
            tags: row.get(4)?, timestamp: row.get(5)?,
...
fn handle_sync_client(mut stream: TcpStream, db_path: &Path) -> Result<()> {
    ...
    let syncer = LanSync { db_path: db_path.to_path_buf() };
    let entries = syncer.export_entries()?;
    let packet = SyncPacket { magic: MAGIC.to_vec(), entries };
...
pub async fn run_lan_sync_command(args: &[String]) -> Result<()> {
    ...
    match subcmd {
        "discover" => {
            ...
            if peers.is_empty() { println!("  No peers found."); }
            for p in &peers { println!("  ✅ {} — {} ({} entries)", p.hostname, p.addr, p.vault_entries); }
        }
        "pull" => {
...
mod tests {
    ...
    fn test_merge_empty() {
        let d = tempdir().unwrap();
        let sync = LanSync { db_path: d.path().join("mem_machine.db") };
    ...
    fn test_merge_entries() {
        ...
        let entries = vec![SyncEntry {
            id: "test-id-001".into(), content: "Prefer bullets".into(),
            app_context: "Word".into(), context_key: "format".into(),
            tags: "lan,test".into(), timestamp: 1700000000, is_ground_truth: true,
        }];
