...
def test_style_profile(temp_db):
    ...
    insert_count = next(t["count"] for t in profile["task_frequencies"] if t["task_type"] == "insert")
...
def test_query_latency_under_300ms(temp_db):
    ...
    for i in range(100):
        client.record_interaction(domain="word", task_type="insert", user_prompt=f"prompt {i}", confidence=0.9)
    
    start = time.time()
    result = client.query(domain="word", limit=5)
    elapsed_ms = (time.time() - start) * 1000
    assert elapsed_ms < 300  # Sub-300ms recall (roadmap spec)

def test_isolation_between_domains(temp_db):
    client = MemMachineClient(db_path=temp_db)
    client.record_interaction(domain="word", task_type="insert", user_prompt="word stuff", style_notes="Use bullets")
    client.record_interaction(domain="excel", task_type="formula", user_prompt="excel stuff", style_notes="Use formulas")
...
def test_record_returns_true_on_success(temp_db):
    client = MemMachineClient(db_path=temp_db)
    result = client.record_interaction(domain="code", task_type="insert", user_prompt="add docstring")
...
def test_user_isolation(temp_db):
    client = MemMachineClient(db_path=temp_db)
    client.record_interaction(domain="word", task_type="insert", user_prompt="user1 memo", user_id="user1", style_notes="formal")
    client.record_interaction(domain="word", task_type="insert", user_prompt="user2 memo", user_id="user2", style_notes="casual")
...
def test_style_vectors_and_centroids(temp_db):
    ...
    client.record_interaction(domain="word", task_type="insert", user_prompt="prompt 1", style_vector=[0.1, 0.2, 0.3])
    client.record_interaction(domain="word", task_type="insert", user_prompt="prompt 2", style_vector=[0.3, 0.4, 0.5])
...
def test_federated_dp_sync(temp_db):
    from sidecar.mem_sync import compute_dp_delta, federated_averaging, log_privacy_budget_to_audit_chain
    
    local_centroid = [0.2, 0.3, 0.4]
    global_centroid = [0.1, 0.1, 0.1]
    
    # Delta should be noised and clipped difference
    delta = compute_dp_delta(local_centroid, global_centroid, clipping_bound=1.0, epsilon=1.0, delta_dp=1e-5)
    assert len(delta) == 3
