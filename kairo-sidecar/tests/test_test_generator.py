...
def scenario_with_keywords(draw):
    ...
    prefix = draw(st.text(alphabet="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ ", min_size=0, max_size=20))
    suffix = draw(st.text(alphabet="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ ", min_size=0, max_size=20))
    ...
    oracle = draw(st.one_of(st.none(), st.text(alphabet="abcdefghijklmnopqrstuvwxyz", min_size=1, max_size=20)))
    fix_budget = draw(st.one_of(st.none(), st.integers(min_value=1, max_value=100)))
    
    scenario = {
        "id": draw(st.text(alphabet="abcdefghijklmnopqrstuvwxyz0123456789", min_size=1, max_size=10)),
        "prompt": prompt,
        "category": category
    }
