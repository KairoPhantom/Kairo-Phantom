...
def test_estimate_difficulty():
    ...
    assert estimate_difficulty("review this NDA contract and add a liability cap", "word") == "complex"
...
def test_run_best_of_n_pdf_scoring(tmpdir):
    ...
    with patch("sidecar.best_of_n.call_with_schema", side_effect=[candidate_1, candidate_2, candidate_3, candidate_1]), \
         patch("sidecar.best_of_n.verify_pdf", side_effect=mock_verify_pdf):
        
        res = run_best_of_n("prompt", DummySchema, "model", "pdf", file_path, mock_master, MagicMock(), N=3)
