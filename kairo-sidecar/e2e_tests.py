...
async def send(action: str, path: str = "", payload: dict = None) -> dict:
    ...
    req = json.dumps({"id": f"e2e-{action}", "action": action,
...
async def test_docx_read_write():
    ...
    record("DOCX: read structure", PASS,
           f"{d['paragraph_count']} paragraphs, {len(d['headings'])} headings")

    # Write via sidecar
    ops = [
        {"action": "insert_after_heading", "heading_text": "Introduction",
         "style": "ListBullet",
         "content": ["Key finding one", "Key finding two", "Key finding three"]},
        {"action": "append", "style": "Normal",
         "content": "Conclusion paragraph added by Kairo."},
    ]
...
async def test_docx_table_insert():
    ...
    ops = [{"action": "insert_table", "style": "Normal",
            "rows": [["Name", "Value", "Change"],
                     ["Revenue", "$1.2M", "+15%"],
                     ["Users", "42,000", "+8%"]],
            "content": ""}]
...
async def test_pdf_extraction():
    ...
    record("TXT: extract_context", PASS,
           f"extracted {len(r['data']['full_text'])} chars")
    os.unlink(tmp)
...
async def test_backup_created():
    ...
    with tempfile.NamedTemporaryFile(suffix=".docx", delete=False) as f:
        tmp = f.name
    doc.save(tmp)
    backup = tmp.replace(".docx", ".docx.kairo_backup")
