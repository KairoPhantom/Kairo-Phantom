...
CUAD_CLAUSE_TAXONOMY: list[tuple[str, list[str], list[str]]] = [
    ...
    # 5
    ("Agreement Name",
     ["this agreement", "master agreement", "service agreement", "license agreement",
      "non-disclosure agreement", "nda", "employment agreement", "purchase agreement"],
     [r"\b(?:this|master|service|license|non-disclosure|employment|purchase)\s+agreement\b",
      r"\bNDA\b"]),
    # 6
    ("Agreement Term",
     ["term of this agreement", "initial term", "term shall be", "duration of this agreement",
      "agreement term"],
     [r"\b(?:initial\s+)?term\s+(?:of|shall\s+be|is)\b", r"\bduration\s+of\s+(?:this\s+)?agreement\b"]),
    # 7
    ("Agreement Type",
     ["service agreement", "license agreement", "non-disclosure agreement", "employment agreement",
      "purchase agreement", "master services agreement", "saas agreement"],
     [r"\b(?:service|license|non-disclosure|employment|purchase|master\s+services?|saas)\s+agreement\b"]),
    # 8
    ("Amendments",
     ["amend", "amendment", "amended", "modify this agreement", "modification",
      "no amendment", "shall not be amended"],
     [r"\bamend(?:ed|ment|s)?\b", r"\bmodif(?:y|ication|ied)\b.*\bagreement\b"]),
    # 9
    ("Anti-Corruption Laws",
     ["anti-corruption", "anti corruption", "fcpa", "bribery", "corrupt practices",
      "foreign corrupt practices"],
     [r"\banti[-\s]?corruption\b", r"\bFCPA\b", r"\b(?:foreign\s+)?corrupt\s+practices\b",
      r"\bbribery\b"]),
    # 10
    ("Audit Rights",
    ...
    # 11
    ("Assignment",
     ["assign", "assignment", "transfer this agreement", "successor", "may not assign",
      "shall not assign"],
     [r"\bassign(?:ment|ed|s)?\b.*\bagreement\b", r"\b(?:may|shall)\s+not\s+assign\b",
    ...
    # 15
    ("Change of Control",
     ["change of control", "acquisition", "merger", "acquired by", "change in control",
      "control of the company"],
     [r"\bchange\s+of\s+control\b", r"\bchange\s+in\s+control\b"]),
    # 16
    ("Compliance with Laws",
     ["compliance with", "comply with all", "applicable laws", "comply with laws",
      "all applicable laws"],
    ...
    # 18
    ("Consideration",
     ["consideration", "in consideration of", "good and valuable consideration",
    ...
    # 21
    ("Data Security",
     ["data security", "security measures", "data protection", "security breach",
      "safeguard", "data breach"],
     [r"\bdata\s+security\b", r"\bsecurity\s+(?:measures|breach|incident)\b",
      r"\bdata\s+protection\b"]),
    # 22
    ("Dispute Resolution",
     ["dispute resolution", "disputes shall be", "resolution of disputes",
      "mediation", "arbitration"],
     [r"\bdispute\s+resolution\b", r"\bresolution\s+of\s+disputes\b",
      r"\bmediation\b", r"\barbitration\b"]),
    # 23
    ("Dispute Resolution Forum",
     ["venue", "jurisdiction", "courts of", "forum", "exclusive jurisdiction",
      "exclusive venue"],
    ...
    # 25
    ("Entire Agreement",
     ["entire agreement", "entire understanding", "supersedes all prior",
      "complete agreement", "entire and exclusive"],
     [r"\bentire\s+agreement\b", r"\bentire\s+understanding\b",
      r"\bsupersedes?\s+all\s+prior\b"]),
    # 26
    ("Exclusivity",
     ["exclusive", "exclusivity", "sole and exclusive", "only provider",
      "exclusive basis"],
     [r"\bexclusiv(?:e|ity)\b"]),
    # 27
    ("Expiration Date",
    ...
    # 28
    ("Force Majeure",
     ["force majeure", "act of god", "circumstances beyond", "unforeseeable",
      "beyond reasonable control"],
     [r"\bforce\s+majeure\b", r"\bact\s+of\s+god\b",
      r"\bbeyond\s+(?:reasonable\s+)?control\b"]),
    # 29
    ("Governing Law",
     ["governed by", "laws of", "governing law", "construed in accordance",
      "governed by the laws"],
     [r"\bgovern(?:ed|ing)\s+by\b", r"\bgoverning\s+law\b",
      r"\bconstrued\s+in\s+accordance\b", r"\blaws\s+of\s+(?:the\s+)?(?:State|state)\b"]),
    # 30
    ("Guarantee",
    ...
    ("Indemnification",
     ["indemnify", "indemnification", "hold harmless", "defend and indemnify",
      "shall indemnify"],
     [r"\bindemnif(?:y|ication|ied)\b", r"\bhold\s+harmless\b",
      r"\bdefend\s+and\s+indemnify\b"]),
    # 32
    ("Insurance",
     ["insurance", "maintain insurance", "certificate of insurance",
      "general liability insurance", "professional liability"],
     [r"\binsurance\b", r"\bmaintain\s+insurance\b",
      r"\bcertificate\s+of\s+insurance\b"]),
    # 33
    ("Intellectual Property Ownership",
     ["intellectual property", "work for hire", "assigns all", "solely owned",
      "all right title and interest", "all right, title, and interest",
      "vest exclusively", "owns all"],
     [r"\bintellectual\s+property\b.*\b(?:own|assign|vest)\b",
      r"\bwork\s+for\s+hire\b",
      r"\ball\s+right[,]?\s+title[,]?\s+and\s+interest\b"]),
    # 34
    ("Joint Intellectual Property",
     ["jointly owned", "joint intellectual property", "joint ownership",
      "jointly developed"],
     [r"\bjoint(?:ly)?\s+(?:owned|intellectual|ownership|developed)\b"]),
    # 35
    ("License Grant",
     ["license grant", "grants a license", "license to use", "licensed",
      "grant of license", "hereby grants"],
     [r"\blicense\s+(?:grant|to\s+use)\b", r"\bgrant\s+of\s+license\b",
      r"\bhereby\s+grants?\b.*\blicense\b"]),
    # 36
    ("Limitation of Liability",
     ["liability shall not exceed", "cap on liability", "limitation of liability",
      "aggregate liability", "limited to fees paid", "maximum liability",
      "liability is limited", "limited to the contract value", "total aggregate liability"],
     [r"\blimitation\s+of\s+liability\b", r"\bliability\s+(?:shall\s+)?not\s+exceed\b",
      r"\baggregate\s+liability\b", r"\bmaximum\s+liability\b",
      r"\bliability\s+is\s+limited\b"]),
    # 37
    ("Minimum Commitment",
     ["minimum purchase", "minimum commitment", "take-or-pay", "minimum order",
      "minimum spend", "minimum volume"],
     [r"\bminimum\s+(?:purchase|commitment|order|spend|volume)\b",
      r"\btake[-\s]or[-\s]pay\b"]),
    # 38
    ("No Waiver",
     ["no waiver", "waiver", "shall not constitute a waiver", "failure to enforce",
      "waiver of any"],
     [r"\bno\s+waiver\b", r"\bwaiver\b.*\b(?:shall\s+not|not\s+constitute)\b",
      r"\bfailure\s+to\s+enforce\b"]),
    # 39
    ("Non-Compete",
     ["non-compete", "non compete", "non-solicitation", "not solicit",
      "competitive activities", "not to solicit", "solicit", "restraint of trade"],
     [r"\bnon[-\s]?compete\b", r"\bnon[-\s]?solicitation\b",
      r"\bnot\s+(?:to\s+)?solicit\b", r"\bcompetitive\s+activities\b"]),
    # 40
    ("Non-Transferability",
     ["non-transferable", "nontransferable", "shall not transfer", "may not transfer",
      "not transferable", "non-assignable"],
     [r"\bnon[-\s]?transfer(?:able|ability)\b", r"\b(?:shall|may)\s+not\s+transfer\b",
      r"\bnot\s+transferable\b"]),
    # 41
    ("Notice",
     ["notice", "written notice", "shall provide notice", "notice period",
      "days prior notice", "days written notice"],
     [r"\b(?:written\s+)?notice\b", r"\bnotice\s+period\b",
...
class CUADExtractor:
    ...
    def enhance_with_llm(
        ...
        try:
            ...
            track = lambda *a, **kw: lambda f: f  # no-op fallback
