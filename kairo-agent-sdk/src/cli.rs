...
pub async fn install_agent(github_url: &str, agents_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let raw_url = github_url.replace("github.com", "raw.githubusercontent.com").to_string() + "/main/manifest.toml";
    ...
    println!("Agent [{}] installed. Restart Kairo or it will hot-reload in <5 seconds.", manifest.agent.name);
    Ok(())
}
